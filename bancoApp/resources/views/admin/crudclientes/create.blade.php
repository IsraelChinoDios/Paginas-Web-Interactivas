...
<head>
  <meta charset="utf-8">
  <title>Nuevo Cliente — bancoApp</title>
...
<body class="min-h-screen bg-gray-50 text-gray-800">
  ...
  <header class="sticky top-0 z-50 border-b border-gray-200/60 bg-white/75 backdrop-blur">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-lg font-semibold leading-none">Nuevo Cliente</h1>
        </div>
      </div>
  ...
  <main class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-10">
    ...
    <form method="POST" action="{{ route($area.'.crudclientes.store') }}"
      ...
      <div>
        ...
        <input id="password" name="password" type="password" required
          class="w-full border border-gray-300 rounded-lg p-2.5
                  focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
        <p class="text-xs text-gray-500 mt-1">Se guardará hasheada automáticamente.</p>
        @error('password') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
      </div>

      <div>
        <label class="block text-sm font-medium mb-1" for="rfc">RFC (13 caracteres)</label>
        <input id="rfc" name="rfc" maxlength="13" value="{{ old('rfc') }}" required
