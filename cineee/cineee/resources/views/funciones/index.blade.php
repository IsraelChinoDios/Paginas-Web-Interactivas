<x-layouts.app :title="__('Funciones')">
    <div class="flex flex-col gap-6">
        <div class="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
            ...
            <flux:modal.trigger name="create-funcion">
                <button
                    ...
                    x-data=""
                    x-on:click.prevent="$dispatch('open-modal', 'create-funcion')"
                >
                    {{ __('Nueva funcion') }}
                </button>
            </flux:modal.trigger>
        ...
        <flux:modal name="create-funcion" :show="$errors->createFuncion->isNotEmpty()" focusable class="max-w-3xl">
            <form method="POST" action="{{ route('funciones.store') }}" class="space-y-6">
                @csrf
                <input type="hidden" name="context" value="create-funcion">

                @include('funciones._form', ['peliculas' => $peliculas, 'salas' => $salas, 'errorBag' => 'createFuncion'])
                ...
                <div class="flex justify-end gap-2">
                    ...
                    <flux:button type="submit" variant="filled">{{ __('Crear funcion') }}</flux:button>
                </div>
            </form>
        </flux:modal>
        ...
        <form method="GET" action="{{ route('funciones.index') }}" class="flex flex-col gap-3 rounded-md border border-neutral-200 bg-white p-4 shadow-sm dark:border-neutral-700 dark:bg-neutral-900 sm:flex-row sm:items-center sm:justify-between">
            ...
            <input
                ...
                placeholder="{{ __('Buscar por fecha, pelicula, sala o tipo') }}"
                    ...
                    @forelse ($funciones as $funcion)
                        <tr>
                            ...
                            <td class="px-4 py-3 text-sm text-neutral-600 dark:text-neutral-200">
                                {{ $funcion->pelicula->nombre ?? __('Sin pelicula') }}
                            </td>
                            <td class="px-4 py-3 text-sm text-neutral-600 dark:text-neutral-200">
                                {{ $funcion->sala->nombre ?? __('Sin sala') }}
                                @if ($funcion->sala && $funcion->sala->sucursal)
                                    <span class="ml-1 text-xs text-neutral-500 dark:text-neutral-400">
                                        ({{ $funcion->sala->sucursal->nombre }})
                                    </span>
                                @endif
                            </td>
                            <td class="px-4 py-3 text-sm text-neutral-600 dark:text-neutral-200">
                            ...
                            <td class="px-4 py-3 text-sm text-neutral-600 dark:text-neutral-200">
                                {{ number_format((float) $funcion->costo, 2) }}
                            </td>
                            <td class="px-4 py-3 text-right text-sm font-medium">
                                <div class="flex justify-end gap-2">
                                    <flux:modal.trigger name="edit-funcion-{{ $funcion->id }}">
                                        <button
                                            type="button"
                                            ...
                                            x-data=""
                                            x-on:click.prevent="$dispatch('open-modal', 'edit-funcion-{{ $funcion->id }}')"
                                        >
                                            {{ __('Editar') }}
                                        </button>
                                    </flux:modal.trigger>
                                    ...
                                    <flux:modal
                                        name="edit-funcion-{{ $funcion->id }}"
                                        :show="$errors->updateFuncion->isNotEmpty() && old('funcion_id') == (string) $funcion->id"
                                        focusable
                                        class="max-w-3xl"
                                    >
                                        <form method="POST" action="{{ route('funciones.update', $funcion) }}" class="space-y-6">
                                            @csrf
                                            @method('PUT')
                                            <input type="hidden" name="context" value="edit-funcion">
                                            <input type="hidden" name="funcion_id" value="{{ $funcion->id }}">

                                            @include('funciones._form', [
                                                'funcion' => $funcion,
                                                'peliculas' => $peliculas,
                                                'salas' => $salas,
                                                'errorBag' => 'updateFuncion',
                                            ])
                                            ...
                                            <div class="flex justify-end gap-2">
                                                ...
                                                <flux:button type="submit" variant="filled">{{ __('Actualizar funcion') }}</flux:button>
                                            </div>
                                        </form>
                                    </flux:modal>

                                    <form action="{{ route('funciones.destroy', $funcion) }}" method="POST" class="inline-flex">
                                        @csrf
                                        @method('DELETE')
                                        <button
                                            type="submit"
                                            ...
                                            onclick="return confirm('{{ __('Eliminar esta funcion?') }}')"
                                        >
                                            {{ __('Eliminar') }}
                                        </button>
                                    </form>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-6 text-center text-sm text-neutral-500 dark:text-neutral-300">
                                {{ __('No hay funciones registradas todavia.') }}
                            </td>
                        </tr>
                    @endforelse
                </tbody>
